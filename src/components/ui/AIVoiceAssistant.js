...
const AIVoiceAssistant = ({ data, setData, setViewDetail }) => {
    ...
    const callGemini = async (text) => {
        return requestQueue = requestQueue.then(async () => {
            ...
            const ctx = {
                parties: (data?.parties || []).slice(0, 50).map(p => ({ id: p.id, name: p.name })),
                items: (data?.items || []).slice(0, 80).map(i => ({ id: i.id, name: i.name, sellPrice: i.sellPrice, category: i.category })),
                openTasks: (data?.tasks || []).filter(t => t.status !== 'Done' && t.status !== 'Converted').slice(0, 30).map(t => {
